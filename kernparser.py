"""
Author: John Thickstun (https://github.com/jthickstun)
"""
...
def parse_humdrum(file, mensural_notation, default_instrument=None):
    ...
    for lineno, line in enumerate(file):
        ...
        linetype = line[
            0
        ]  # either ! for comment, * for interpretion, or other for data
        ...
        parts = 0
        for spine in spines:
            if spine.exclusive_interpretation == "kern":
                parts += 1
        if parts > 6:
            error("too many parts ({})".format(parts), lineno)
        ...
        if linetype == "*":
            ...
            for toke in tokens:
                if (
                    toke[0] != "*"
                ):  # if linetype is interpretation, all tokens must be an interpretation
            ...
            if exclusive:
                for toke in tokens:
                    ...
                    if toke[0] != "*" or toke[1] != "*":
                        error(
                            "attempt to mix exclusive and tandem interpretations",
                            lineno,
                        )
                ...
                for i, toke in enumerate(tokens):
                    ...
                    if spines[i].exclusive_interpretation:
                        error(
                            "attempt to change a previously-defined exclusive interpretation",
                            lineno,
                        )
                ...
                for toke in tokens:
                    if toke == "*":
                        continue  # null is okay
                    elif toke[1] == "*":
                        error(
                            "attempt to mix tandem and exclusive interpretations",
                            lineno,
                        )
                    ...
                    for i, toke in enumerate(tokens):
                        if toke == "":
                            ...
                            if not skip:
                                ...
                                if (
                                    new_spines[-1].exclusive_interpretation
                                    != spines[i].exclusive_interpretation
                                ):
                                    error(
                                        "joining spines with inconsistent interpretations",
                                        lineno,
                                    )
                                if spines[i].exclusive_interpretation == "kern":
                                    if (
                                        abs(
                                            new_spines[-1].frontier - spines[i].frontier
                                        )
                                        > 0.0001
                                    ):
                                        error(
                                            "joining spines with inconsistent frontiers",
                                            lineno,
                                        )
                            skip = True
                            joins += 1
                            ...
                            s.frontier = spines[
                                i
                            ].frontier  # should really be global_frontier??
                            new_spines.append(s)
                            skip = False
                            intros += 1
                        elif toke == "x":
                            ...
                            error(
                                "attempt to combine control spines with other interpr. directives",
                                lineno,
                            )
                    spines = new_spines
                # format-specific control tokens
                    ...
                    for i, toke in enumerate(tokens):
                        ...
                        if (
                            instr is not None
                            and re.sub(r"\d+", "", instr).strip()
                            in instrument_map.keys()
                        ):
                    ...
                    if validate_time_signatures:
                        ...
                        if ts:
                            ...
                            if ts.group(3):
                                d /= int(str(ts.group(3))[1:])  # rational denominator
                            if d == 0:
                                d = 0.5  # breve
                            tpm = n / d
                            if time_per_measure is None:
                                time_per_measure = tpm  # init
                            ...
                            for i, toke in enumerate(tokens):
                                ...
                                if (
                                    toke == ""
                                ):  # null is okay if we're consistent (e.g. introducing new spine)
                                    if tpm != time_per_measure:
                                        error("inconsistent time signatures", lineno)
                                    continue

                                ts = re.search(r"(\d+)/(\d+)(%\d+)?", toke)
                                if not ts:
                                    error(
                                        "invalid time signature syntax: '{}'".format(
                                            toke
                                        ),
                                        lineno,
                                    )
        ...
        # parse a line of data
            ...
            for i, spine in enumerate(spines):
                if spine.exclusive_interpretation != "kern":
                    continue
                if channel > 5:
                    error("channel overflow", lineno)
                for src in spine.source:
                    if channel_map[src] > 5:
                        error("channel overflow", lineno)
            ...
            if linetype == "=":  # barline
                ...
                # (notes can't span the line;
                # if they do they should be written as separate notes with ties)
                frontier = spines[0].frontier
                for spine in spines:
                    if spine.exclusive_interpretation != "kern":
                        continue
                    if abs(spine.frontier - frontier) > 0.0001:
                        error(
                            "spines out of alignment: {}".format(
                                [s.frontier for s in spines]
                            ),
                            lineno,
                        )
                ...
                # get time signatures working if we want good "location" data
            ...
            new_durations = -1 * np.ones(
                [6]
            )  # durations: -1 sentinal indicates unused channel, 0 indicates continuation
            ...
            for i, toke in enumerate(tokens):
                ...
                if spines[i].instrument is None:
                    if default_instrument is not None:
                        spines[i].instrument = default_instrument
                    else:
                        error(
                            "data record in spine {} with no associated instrument".format(
                                i
                            ),
                            lineno,
                        )
                ...
                instr = (
                    spines[i].instrument
                    if spines[i].staff is None
                    else spines[i].instrument + spines[i].staff
                )
                if "piano" in instr:
                    instr = (
                        "piano"  # forget about staves; group all piano notes together
                    )

                if toke == ".":  # null
                    if not spines[i].rest:
                        instrument_continuations[instr] += 1
                ...
                if frontier:
                    if abs(spines[i].frontier - frontier) > 0.0001:
                ...
                for item in chord:
                    ...
                    if rational_duration:
                        ...
                        )  # not a typo: durations are inverted
                    else:
                        duration = re.search(r"(\d+)", item)
                        if not duration:
                            error("note with unspecified duration", lineno)

                        base_value = duration.group(1)
                        if (
                            int(base_value) == 0
                        ):  # extended duration (breve, longa, maxima, etc.)
                            base_duration = 2 ** (len(base_value))
                        else:
                            # simple duration: fraction of a whole note
                            # (e.g. 4 == quarter note, 8 == eighth note)
                            base_duration = 1 / float(base_value)

                    # speed up older scores to modern conventions
                    if mensural_notation:
                        base_duration /= 4
                    ...
                    for d in range(dots):
                        duration += base_duration / (2 ** (d + 1))

                    if chord_duration and chord_duration != duration:
                        error("chord specified with inconsistent durations", lineno)
                    ...
                    if "r" in item:  # it's a rest
                        ...
                        if not note:
                            error("unhandled non-note non-rest item", lineno)
                        note = note.group(1)
                        ...
                        midi_code = (
                            60 + 12 * (octave - 4) + pitch_map[pitch] + sharp - flat
                        )
                ...
                part_representation[instr].append(
                    (new_event[channel_map[i]], chord_duration, spines[i].frontier)
                )

                spines[i].frontier += chord_duration
                new_durations[channel_map[i]] = chord_duration

            # for intruments with a new note, count how many concurrent notes in
            # that instrument's part are chopped
            part_overlap += sum(
                [instrument_continuations[instr] for instr in instrument_notes.keys()]
            )
            full_overlap += sum(
                [
                    instrument_continuations[instr]
                    for instr in instrument_continuations.keys()
                ]
            )

            if not null_event:  # event can be null if e.g. event is full of grace notes
                if len(event_representation) == 0:  # first event?
                    flow = np.eye(
                        6
                    )  # flow is undefined; for modeling convenience, make it the identity
                event_representation.append(
                    (new_event, new_durations, flow)
                )  # log the event

            if frontier is not None:
                for i, toke in enumerate(tokens):
                    if (
                        toke == "." and spines[i].exclusive_interpretation == "kern"
                    ):  # make sure that nulls are consistent with the frontier
